...
void aes::aes256cbc_encrypt(std::vector <unsigned char> & ctext, std::vector <unsigned char> msg, std::vector <unsigned char> key, std::vector <unsigned char> iv) {

    // need fresh iv every message
    EVP_CIPHER_CTX_free_ptr ctx(EVP_CIPHER_CTX_new(), ::EVP_CIPHER_CTX_free);

    int rc = EVP_EncryptInit_ex(ctx.get(), EVP_aes_256_cbc(), NULL, key.data(), iv.data());
    if (rc != 1)
...
void aes::aes256cbc_decrypt(std::vector <unsigned char> & msg, std::vector <unsigned char> ctext, std::vector <unsigned char> key, std::vector <unsigned char> iv){

//    std::cout << "aes256cbc_decrypt ctext size " << ctext.size() << std::endl;
    EVP_CIPHER_CTX_free_ptr ctx(EVP_CIPHER_CTX_new(), ::EVP_CIPHER_CTX_free);
    int rc = EVP_DecryptInit_ex(ctx.get(), EVP_aes_256_cbc(), NULL, key.data(), iv.data());
    if (rc != 1)
